...
UniPC can accelerate both the conditional and unconditional sampling by stable-diffusion. After setting the environment and downloading the required models following the original instructions in stable-diffusion, sampling with our UniPC can be simply achieved by adding a `--uni_pc` option in the sampling scripts. For example (`sample.sh`):
...
To use UniPC in other sampling scripts, please add one line:
